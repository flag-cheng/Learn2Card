...
const App = () => {
  ...
  const generateFromUpload = useCallback(async (file: File) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const handleDeckJsonFileChange = useCallback(async (file: File) => {
    setLoading(true);
    setError(null);
    try {
        ...
        <div className="main-layout">
          <aside className="sidebar">
            <section className="panel">
              <div className="panel-title">上傳與載入</div>
              <div className="source-controls">
                ...
                <div className="control-row">
                  <label className="file-label">
                    <span className="file-label-text">上傳檔案（Markdown/純文字）</span>
                    <input
                ...
                <div className="control-row">
                  <label className="file-label">
                    <span className="file-label-text">進階：選擇本機 deck.json</span>
                    <input
                      className="file-input"
                      type="file"
                      ...
                      onChange={(e) => {
                        const file = e.target.files?.[0];
                        if (!file) return;
                        void handleDeckJsonFileChange(file);
                        e.currentTarget.value = "";
                      }}
                      disabled={loading}
                    />
                ...
                <div className="hint-text">
                  提示：上傳功能需要先啟動後端（預設 `http://localhost:8000`）；翻卡也支援鍵盤 ← / →
                </div>
              </div>
            </section>
